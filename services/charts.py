...
def build_body_weight_chart(logs: List[dict]) -> go.Figure:
    """Vücut ağırlığı trendi: logs = [{"date": "YYYY-MM-DD", "weight_kg": float}, ...] (tarih sıralı)."""
    if not logs:
        fig = go.Figure()
        fig.add_annotation(text="Henüz vücut ağırlığı kaydı yok", xref="paper", yref="paper", x=0.5, y=0.5, showarrow=False)
        fig.update_layout(**PLOTLY_LAYOUT)
        return fig
    ...
    fig.add_trace(
        go.Scatter(
            ...
            marker=dict(size=8),
            name="Vücut ağırlığı (kg)",
        )
    )
    fig.update_layout(
        **PLOTLY_LAYOUT,
        title=dict(text="Vücut Ağırlığı Trendi", font=dict(size=14)),
        xaxis_title="Tarih",
        yaxis_title="kg",
        showlegend=False,
    )
...
def build_1rm_line_chart(dates: List[str], estimated_1rms: List[float], exercise_name: str) -> go.Figure:
    """Tahmini 1RM (tek tekrar max) trendi çizgi grafik."""
    if not dates or not estimated_1rms:
        fig = go.Figure()
        fig.add_annotation(text="Veri yok", xref="paper", yref="paper", x=0.5, y=0.5, showarrow=False)
        fig.update_layout(**PLOTLY_LAYOUT)
        return fig
    fig = go.Figure()
    fig.add_trace(
    ...
    fig.update_layout(
        **PLOTLY_LAYOUT,
        title=dict(text=f"{exercise_name} – Tahmini 1RM (Epley)", font=dict(size=14)),
        xaxis_title="Tarih",
        yaxis_title="kg",
        showlegend=False,
    )
