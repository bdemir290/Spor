...
def main():
    ...
    with st.container():
        ...
        protein_value = (current["protein_goal_met"] if current else False)
        vitamin_value = (current.get("vitamin_taken", False) if current else False)

        new_protein = st.checkbox(
            "Yeterince protein aldım (Örn: 120g)",
            value=protein_value,
            key="protein_checkbox",
        )
        new_vitamin = st.checkbox(
            "Vitamin aldım",
            value=vitamin_value,
        ...
        if st.button("Kaydet"):
            try:
                ok = set_nutrition_for_date(today_str, new_protein, new_vitamin)
                if ok:
                    st.cache_data.clear()
